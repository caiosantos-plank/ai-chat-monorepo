                ...
                <nav className="flex-1 overflow-y-auto p-2 space-y-1">
                    {chats?.length ? (
                        chats.map((chat, index) => {
                            const isActive = (activeChatId && chat.id === activeChatId) || pathname.endsWith(`/home/${chat.id}`);
                            return (
                                <Link
                                    key={chat.id}
                                    href={`/home/${chat.id}`}
                                ...
                                >
                                    <div className="flex items-center justify-between gap-2">
                                        ...
                                        <div className="flex items-center gap-2">
                                            <svg
                                                className={`w-4 h-4 text-muted-foreground/70 opacity-0 group-hover:opacity-100 transition-opacity ${isActive ? "opacity-100" : ""}`}
                                                fill="none"
                                                stroke="currentColor"
                                                viewBox="0 0 24 24"
                                                aria-hidden="true"
